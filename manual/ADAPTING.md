...
This pack ships `node`, `jvm`, `php`, **`android`** and **`python`** branches. Android is a
declared stack, not a detected one: a Gradle project probes as `jvm`, so an Android repo
declares `"stack": ["jvm", "android"]` and gets both branches — the JVM toolchain advice
plus the emulator/`adb`/`minSdk` specifics.

The `python` branch covers what the engine detects as Python (engine ≥ 0.6.0): any
project with `pyproject.toml` (PEP 621 / Poetry / PDM / uv / Hatch), `requirements*.txt`
(incl. `requirements/*.txt`), `setup.py` / `setup.cfg`, `Pipfile`, `environment.yml`, or a
`uv.lock` / `poetry.lock` / `pdm.lock` — and repos of plain `*.py` scripts (root,
`scripts/`, `bin/`) with no manifest at all. What the branches know: environments via
`uv sync` + `uv run` or a venv (`python -m venv`, `uv venv`); `src/` and flat layouts;
Django (`manage.py`, `apps.py`, `urls.py`, migrations), FastAPI / Flask route decorators,
DRF routers; the verify loop `ruff check` → `ruff format --check` → `mypy` (or the
configured checker) → `pytest`, with `compileall` + `unittest discover` as the floor for
script repos; lock files regenerated by the tool, never by hand. Every command in the
branches was run on a real project before it was written down.
